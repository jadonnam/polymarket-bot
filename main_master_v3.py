...
def _news_score(title, desc=""):
    ...
    score = 25
    if _contains(text, ["환율", "usd", "fx", "달러", "won"]): score += 28
    if _contains(text, ["oil", "wti", "crude", "brent", "유가"]): score += 26
    if _contains(text, ["war", "attack", "missile", "전쟁", "공습", "이란", "israel", "iran", "비행기", "격추"]): score += 28
    if _contains(text, ["fed", "cpi", "inflation", "yield", "금리", "물가"]): score += 20
...
def _poly_score(question, volume, yes_price):
    ...
    try: p = float(yes_price)
    except Exception: p = 0.5
    if v >= 20000000: score += 40
    elif v >= 10000000: score += 32
    elif v >= 5000000: score += 24
    elif v >= 1000000: score += 14
    if p >= 0.90 or p <= 0.10: score += 18
    elif p >= 0.80 or p <= 0.20: score += 10
    if _contains(text, ["strait of hormuz", "hormuz", "호르무즈"]): score += 14
    if _contains(text, ["oil", "wti", "crude", "유가", "dollar", "달러", "fx", "환율"]): score += 12
    if _contains(text, ["war", "attack", "iran", "israel", "전쟁", "이란", "비행기", "격추"]): score += 14
    if _contains(text, ["btc", "bitcoin", "비트", "eth", "ethereum"]): score += 10
    return min(score, 100)
...
def fetch_news_top5():
    ...
    for a in items:
        ...
        if not title:
            continue
        ranked.append({"title": _smart_label(title), "score": _news_score(title, desc), "raw_title": title})
...
def fetch_poly_top5():
    ...
    for m in markets:
        ...
        if label in seen:
            continue
        seen.add(label)
        ranked.append({"title": label, "score": _poly_score(q, m.get("volume", 0), m.get("yes_price", 0.5)), "raw_title": q})
    ranked.sort(key=lambda x: x["score"], reverse=True)
    return ranked[:5]
...
def choose_breaking_candidate(news_top, poly_top):
    ...
    if news_top:
        ...
        if best["score"] >= BREAKING_NEWS_MIN_SCORE:
            candidates.append({"source": "news", "score": best["score"], "raw_title": best["raw_title"], "title": breaking_headline(best["raw_title"])})
    if poly_top:
        best = poly_top[0]
        if best["score"] >= BREAKING_POLY_MIN_SCORE:
            candidates.append({"source": "polymarket", "score": best["score"], "raw_title": best["raw_title"], "title": breaking_headline(best["raw_title"])})
    if not candidates:
        return None
...
def send_regular_rank_cards(news_top, poly_top):
    ...
    paths = create_rank_set(news_items=news_top, poly_items=poly_top, out_dir=OUT_DIR)
