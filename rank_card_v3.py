...
def _hero_note(label: str, page_type: str) -> str:
    if page_type == "news":
        table = {
            "유가 상방 압력": "에너지 가격 이슈 재부각", "휴전 기대 확대": "지정학 완화 기대 반영",
            "비트코인 강세 유지": "위험자산 선호 재확대", "달러 강세 유지": "환율 부담 지속",
            "금리 완화 기대": "정책 기대 반영", "안전자산 선호": "방어 자산 선호 확대",
        }
    elif page_type == "poly":
        table = {
            "유가 상단 도전": "상방 시나리오 베팅 집중", "휴전 베팅 확대": "정치 이벤트 자금 유입",
            "호르무즈 정상화 기대": "물류 정상화 기대 반영", "트럼프 변수 확대": "정책 변수 베팅 확대",
            "비트코인 상단 테스트": "상단 돌파 기대 유입",
        }
    else:
        table = {
            "유가 상방 압력": "실물 체감 연결 구간", "환율 변동성 확대": "달러 영향 확산",
            "비트코인 강세 유지": "위험 선호 유지", "금 선호 강화": "안전자산 선호 증가",
            "금리 부담 확대": "고금리 부담 재부각",
        }
...
def _draw_rows(draw, items, accent):
    left, right, label_x, start_y, row_gap = 86, W - 86, 170, 510, 166
